...
class Indicator {
  ...
  const Indicator(this.title, this.subTitle);
}
...
class AppDatabase {
  ...
  static List<Indicator> get indicatorItems {
    final List<Indicator> items = [];
    for (int i = 1; i <= 4; i++) {
      items.add(Indicator('Read the article you want instantly',
          'You can read thousands of articles on Blog Club, save them in the application and share them with your loved ones.($i)'));
    }
    return items;
  }
}
